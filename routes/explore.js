...
cloudinary.config({
  cloud_name: "ddfqrmndp",
  api_key: "865569447617784",
  api_secret: "kxcxk14H8QbKbNr0BSX0Uy5_LAM",
});
        ...
        .exec((err, Posts) => {
          if (err) {
            ...
            categories.forEach((category) => {
              if (!counts.hasOwnProperty(category.name)) category.count = 0;
              else {
...
router.post("/", middleware.isLoggedIn, upload.single("image"), (req, res) => {
  cloudinary.uploader.upload(req.file?.path, (result) => {
    ...
    req.body.post.image = result.secure_url ?? "anhdaidien.png";
